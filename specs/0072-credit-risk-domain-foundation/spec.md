...
| REQ-014 | Any credit workflow that produces or supports a decision about an identifiable consumer applicant or borrower shall be required by contract to: emit principal-reason codes traceable to the model inputs that drove the outcome; record the decision policy version, cutoff, and overrides applied; keep protected-class attributes out of model features while permitting their use in segregated fairness testing; and expose a disparate-impact test hook. Attribute absence is a necessary but **not** sufficient fairness control — the substantive obligations are in REQ-020, and this requirement shall not be represented as fair-lending compliance on its own. A workflow that cannot meet these shall be declared decision-support-only and prohibited from being the sole basis of an adverse action. | must |
...
| REQ-020 | Any workflow classified as a consumer decision under REQ-014 shall additionally declare substantive fairness testing: (a) a protected-class basis for testing, either directly observed or estimated by a named method whose limitations are recorded, marked testing-only and prohibited from appearing in the feature set; (b) an outcome-disparity metric with an institution-supplied threshold, measured at the decision cutoff actually applied; (c) per-feature association with the protected-class basis, recorded so that a proxy is visible rather than hidden; and (d) an obligation to search for a less discriminatory alternative when the threshold is breached, recording either the alternative adopted or the business-need rationale for retaining the model. These results shall be an artifact versioned with the model, not a one-off study. A path that cannot meet these shall be `decision_support_only`. | must |
...
| NFR-008 | Fairness testability | Fairness and adverse action contracts are expressed so they can be tested mechanically on synthetic fixtures: reason codes are derivable, protected attributes are absent from feature sets, disparity metrics are computable at the applied cutoff, per-feature proxy association is recorded, and a threshold breach mechanically obliges a less-discriminatory-alternative record. Mechanical testability establishes that the controls exist and are wired, never that a model is lawful. |
...
| AC-023 | Given a workflow contract declared to support a consumer credit decision, when it is validated, then it declares a protected-class testing basis (observed, or a named estimation method with recorded limitations), a disparity metric with a supplied threshold, per-feature proxy-association recording, and a less-discriminatory-alternative obligation; a path missing any of these validates only as `decision_support_only`. | REQ-020, NFR-008 |
| AC-024 | Given a protected-class estimate produced for testing, when it appears anywhere in a model's feature set, then validation fails; and given a synthetic outcome fixture whose disparity metric breaches the declared threshold, when fairness testing runs, then the breach is reported and a less-discriminatory-alternative record is required before the path can remain sole-basis. | REQ-020, NFR-008 |
...
| RISK-003 | A consumer-facing decision path ships without deployable reason codes or fairness testing. | Regulatory exposure, consumer harm, and an undeployable model at the last gate. | REQ-014 makes reason codes, policy versioning, protected-attribute segregation, and a disparate-impact hook structural; REQ-020 adds the substantive half (disparity measurement, proxy visibility, less-discriminatory-alternative search); a contract that cannot meet them is decision-support-only. |
...
| RISK-010 | Attribute absence is mistaken for fairness. A model containing none of the protected attributes still encodes them through correlated features (geography, education, employer, device), and a contract that only checks a feature list certifies nothing. | A decline pattern that fails a fair-lending exam passes every check the SDK performs, with the SDK's own structure supplying false comfort. | REQ-020 requires disparity measured at the applied cutoff, per-feature proxy association recorded, and a less-discriminatory-alternative search on breach. REQ-014 now states in its own text that attribute absence is necessary but not sufficient. This risk was found by audit of REQ-014 after implementation, not before; the gap register records that history rather than erasing it. |
...
- Open question: which protected-class estimation method should `0074` name as its
  default testing basis, and under what recorded limitations? Estimation is standard
  practice where the attribute is not lawfully collected, and every method carries
