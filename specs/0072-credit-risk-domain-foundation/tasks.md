...
| T-011 | Author the `agents/credit_risk/` charter: roster, per-agent responsibility, inputs/outputs, serving workflow, and the coverage row justifying each agent. Create only the agents the matrix justifies. | REQ-011 | in-progress | The charter (`workflows.json`'s `participating_agents`) names the roster and gates creation on a coverage row. Three agents now exist: `credit_document_analyst` (via `0077`, once `capability.document_intelligence` reached `reference_runtime`), `counterparty_limits` (via `0073`), and `fair_lending_review` (via `0074`). `test_credit_agent_creation_stays_gated_on_coverage_AC_011` asserts exactly those three exist and that `obligor_rating` and `retail_underwriting` deliberately do not, since rating/PD estimation and consumer scoring both stay `0026`-plugin-only. Remaining named agents (ECL, stress testing) stay uncreated pending their own child specs. |
...
| T-016 | Complete two-part human review — credit practitioner review of measures, conventions, lifecycles, and decision paths, and a model-validation review of temporal and numerical cases — recording reviewer handles, scope, dates, and dispositions. | REQ-019, NFR-003, NFR-004 | done | Reviewer named 2026-09-11: Joshua Lutkemuller, CFA (repository owner and approver; a self-attestation, not an independent third-party credit officer's sign-off — see `G-0072-001`). `0073` closed `G-0072-002` and `0074` closed `G-0072-005`, the two remaining high-severity gaps; **all 109 records are now `reviewed`**, `blocked_pending_gap` reads `0`. `test_promotion_state_matches_gap_register_AC_020` asserts this exactly, and `test_a_record_naming_a_still_open_high_gap_cannot_be_reviewed_AC_020` proves the enforcement mechanism itself, not just today's data. |
