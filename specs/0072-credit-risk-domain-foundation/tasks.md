...
| T-011 | Author the `agents/credit_risk/` charter: roster, per-agent responsibility, inputs/outputs, serving workflow, and the coverage row justifying each agent. Create only the agents the matrix justifies. | REQ-011 | in-progress | The charter (`workflows.json`'s `participating_agents`) names the roster and gates creation on a coverage row. Two agents now exist: `credit_document_analyst` (via `0077`, once `capability.document_intelligence` reached `reference_runtime`) and `counterparty_limits` (via `0073`, once `capability.counterparty_limits` did). `test_credit_agent_creation_stays_gated_on_coverage_AC_011` asserts exactly those two exist and that `obligor_rating` deliberately does not, since rating/PD estimation stays `0026`-plugin-only. Remaining named agents (retail underwriting, ECL, stress testing) stay uncreated pending their own child specs. |
